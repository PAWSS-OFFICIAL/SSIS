        ...
        with conn.cursor() as cursor:
            ...
            if user.role == 'Student':
                ...
                if not cursor.fetchone():
                    ...
                    cursor.execute("UPDATE users SET parent_id = %s WHERE id = %s", (parent_user_id, user_id))
            
            conn.commit()
            trigger_webhook("user.created", {"user_id": user_id, "username": user.username, "role": user.role})
...
async def export_users(
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            query = """
                SELECT id, username, email, role, name, idno, department, year, section, created_at
...
async def export_grades(
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            query = """
                SELECT g.id, u.name as student_name, u.idno as usn, c.name as course_name,
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            ...
            if request.student_ids:
                cursor.execute("""
                    SELECT id, name, idno as usn, department, year, section
...
async def get_weekly_parent_report(token: dict = Depends(require_role('Parent'))):
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            ...
            # Get student info
            cursor.execute("SELECT name, department, year FROM users WHERE id = %s", (student_id,))
            student = cursor.fetchone()
...
class ThemeConfig(BaseModel):
    ...
    university_name: str = "JAIN University"
...
CHATBOT_KNOWLEDGE = {
    ...
    "technical issue": "For technical issues, please contact the IT support team or email support@jainuniversity.ac.in",
...
async def chatbot_message(request: ChatMessage):
    ...
    # Default response
    if not response:
        response = "I'm not sure about that. You can contact support@jainuniversity.ac.in for more help or try asking about: password reset, attendance, grades, assignments, quizzes, timetable, leave requests, or technical issues."
    ...
    return {
        "response": response,
        "suggested_actions": [
            {"label": "Contact Support", "action": "email", "value": "support@jainuniversity.ac.in"},
